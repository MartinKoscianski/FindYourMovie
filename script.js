  ...
  try {
    ...
    if(id === false) {
      ...
      if( filters === "all") {
        ...
        url = `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&language=fr-FR&sort_by=popularity.desc&include_adult=false&include_video=true&page=${randomPage}`;
      }
...
async function searchInDB(text) {
  crash = 0;
  await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&language=fr-FR&query=${encodeURIComponent(text)}&page=1&include_adult=false`).then(response => {
...
document.querySelectorAll('button').forEach(button => {
  button.addEventListener('click', () => {
    if(search_mode || isLoading) return;
